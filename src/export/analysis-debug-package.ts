...
export function createAnalysisDebugReadme(): string {
  return [
    ...
    '',
    'source-audio.* contains the exact encoded audio bytes analyzed by SingScope.',
    'diagnostics.json contains the versioned detector configuration, every raw contour frame,',
    'accepted and rejected pitch candidates, confidence, RMS, peak, gap reasons, and estimated notes.',
    'contour.csv and estimated-notes.csv are readable copies of the same analysis evidence.',
    ...
    'It never uploads automatically. Inspect it and share it only when you choose to.',
    'Attaching it to ChatGPT, email, or another service uploads it to that recipient.',
