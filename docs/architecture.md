...
A Manual project does not require imported backing audio. When no backing asset exists, its immutable note revision, transpose, alignment, durations, and gaps deterministically produce a bounded local PCM WAV in a same-origin static worker for the same Safari media-element transport. The 8 kHz mono guide covers the detector's 80–1,200 Hz range; even the 20-minute maximum remains below the 32 MiB WAV and 96 MiB conservative peak-memory budgets. The generated guide is virtual rather than a stored binary asset, so editing cannot leave stale guide audio and a project backup can reconstruct it entirely from cross-field-validated target metadata. An explicitly imported backing asset continues to take precedence.
