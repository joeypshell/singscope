...
- No mixed-song vocal isolation or reliable melody extraction is provided. Imported backing audio is playback material, not an authoritative target.
- Home Screen and Safari projects may live in different storage containers; transfer requires backup/export and import.
- Capture is foreground-only. Lock, app switch, Siri/call, route loss, stopped mic track, or AudioContext interruption ends the take as recoverable partial data.
...
- MediaRecorder format is runtime-selected. iPhone usually provides encoded MP4/AAC, but actual browser support is authoritative.
- YIN is monophonic and may misread breath noise, consonants, vibrato extremes, polyphony, room echo, or tones near range boundaries. Low-confidence ranges are not scored.
