...
- The setup verification chart can replay the exact analyzed source and shows the accepted source contour, confidence breaks, raw candidates, gap reasons, and editable note blocks. A newly analyzed source can produce an opt-in local debug ZIP with the full raw result, but that extra diagnostic draft is memory-only and disappears after leaving setup; saved projects do not retain a second copy of the debug archive. Practice takes retain their raw candidates and gap reasons.
