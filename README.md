...
### Reporting an audio or analysis bug

Immediately after analyzing a newly recorded or uploaded source, open **Report a missed-note bug**. For a grainy, incomplete, or incorrectly detected Practice recording, finish the take, open Review, and expand **Report recording problem**. Enter the number of notes you played, choose the audio route, and briefly describe the mismatch. Read the exact-audio warning, then tap **Send bug report**. SingScope prepares the diagnostic archive locally, obtains a short-lived delivery ticket, performs a small cancellable proof of work on the device, and sends it directly to the configured Supabase report service. A successful response displays a report ID that can be quoted in follow-up.

Nothing is sent merely by recording, analyzing, reviewing, opening the panel, or entering the optional fields. The explicit Send action uploads a ZIP containing the exact affected audio, every retained accepted/rejected detector frame and gap reason, estimated-note segmentation, detector/build settings, applied capture settings, bounded runtime counters, and Safari user-agent/viewport details. It excludes the project title, original filename, microphone name/ID, lyrics, unrelated projects, and local storage identifiers. Reports are stored in a private Supabase bucket and are intended to expire after 30 days by default. If no receipt returns, the app accurately says delivery is unconfirmed—the service may already have committed it—and offers an idempotent **Retry sending report** plus **Save debug package** so the evidence is not lost.
